...
path_mframe = f'{path_base}/master_frame_{n_binning}x{n_binning}'
...
rawlist = sorted(glob.glob(f'{path_raw}/2???-??-??_{n_binning}x{n_binning}'))
#------------------------------------------------------------
			...
			if len(_imagearr) > 0:
				...
				for gg, (group, indx_group) in enumerate(zip(groups, index_groups)):
					...
					if len(group) == 0:
						...
						#	Image Combine
						t0_com = time.time()
						swarpcom = f"swarp -c {path_config}/7dt_{n_binning}x{n_binning}.swarp @{path_imagelist} -IMAGEOUT_NAME {comim} -RESAMPLE_DIR {path_data} -CENTER_TYPE MANUAL -CENTER {center}"
						print(swarpcom)
						os.system(swarpcom)
