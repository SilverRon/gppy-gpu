...
rawlist = sorted(glob.glob(f'{path_raw}/2???-??-??_{n_binning}x{n_binning}_gain2750'))
#------------------------------------------------------------
...
for stack_image in stacked_images:
	...
	_reference_images = []
	for ref_src in ['7DT', 'PS1']:
		_reference_images += glob.glob(f"{path_ref_frame}/ref*{ref_src}*{obj}*fits")
...
#	Image Subtraction
#======================================================================
for ss, (inim, refim, inmask_image, refmask_image) in enumerate(zip(stacked_images, reference_images, sci_mask_images, ref_mask_images)):
	if refim != None:
		#	Subtraction Command
		subtraction_com = f"{path_subtraction} {inim} {refim} {inmask_image} {refmask_image}"
		print(subtraction_com)
		os.system(subtraction_com)
		#	Outputs
		hdim = inim.replace("fits", "subt.fits")
		hcim = inim.replace("fits", "conv.fits")
		#	Photometry Command for Subtracted Image
		phot_subt_com = f"{path_phot_sub} {hdim} {mask_image}"
		print(phot_subt_com)
		os.system(phot_subt_com)
		#	Transient Search Command
		search_com = f"{path_find} {inim} {refim} {hcim} {hdim}"
		print(search_com)
