...
class Calibration:
    ...
    def _load_mbdf(self):
        ...
        if selection == "default":  # use links under self.config.preprocess
            mbias_file = read_link(self.config.preprocess.mbias_link)
            ...
            mdark_file = read_link(self.config.preprocess.mdark_link)
            ...
            mflat_file = read_link(self.config.preprocess.mflat_link)
            ...
            if (
                ...
                == read_link(self.config.preprocess.mbias_link)
                and self.config.preprocess.mdark_file
                == read_link(self.config.preprocess.mdark_link)
                and self.config.preprocess.mflat_file
                == read_link(self.config.preprocess.mflat_link)
            ):
