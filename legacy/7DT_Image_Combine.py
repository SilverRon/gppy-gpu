		...
		if len(_imagearr) > 0:
			...
			for gg, (group, indx_group) in enumerate(zip(groups, index_groups)):
				...
				if len(group) == 0:
					...
					#	Image Combine
					t0_com = time.time()
					swarpcom = f"swarp -c {path_config}/7dt.swarp @{path_imagelist} -IMAGEOUT_NAME {comim} -CENTER_TYPE MANUAL -CENTER {center}"
					# swarpcom = f"swarp -c {path_config}/7dt.bkgsub.swarp @{path_imagelist} -IMAGEOUT_NAME {comim} -CENTER {center}"
					# swarpcom = f"swarp -c {path_config}/7dt.bkgsub512.swarp @{path_imagelist} -IMAGEOUT_NAME {comim} -CENTER {center}"
					print(swarpcom)
					os.system(swarpcom)
