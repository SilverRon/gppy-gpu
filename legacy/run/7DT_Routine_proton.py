...
#	Path
#------------------------------------------------------------
path_base = '/data/large_data_proton/factory'
...
path_phot_mp = './phot/gregoryphot_7DT_proton.py'
...
# path = '/data/large_data_proton/factory/../obsdata/7DT01/2023-00-00'
# path = '/data/large_data_proton/factory/../obsdata/7DT01/2023-10-15'
tdict = dict()
starttime = time.time()
...
if darknumb != 0:
	...
	for i, exptime in enumerate(darkexptimelist):
		...
		for dd, _dinim in enumerate(dimlist): print(f"[{dd:0>2}] {os.path.basename(_dinim)}")
...
for filte in np.unique(objtbl['filter']):
	for exptime in np.unique(objtbl['exptime'][objtbl['filter']==filte]):
		...
		ofc.write(outfnamelist, overwrite=True)
		del ofc
...
com = f'python {path_phot} {path_data} {ncore}'
print(com)
os.system(com)
			...
			if len(_imagearr) > 0:
				...
				for gg, (group, indx_group) in enumerate(zip(groups, index_groups)):
					...
					if len(group) == 0:
						...
						#	Image Combine
						t0_com = time.time()
						swarpcom = f"swarp -c {path_config}/7dt.swarp @{path_imagelist} -IMAGEOUT_NAME {comim} -RESAMPLE_DIR {path_data} -CENTER_TYPE MANUAL -CENTER {center}"
						print(swarpcom)
						os.system(swarpcom)
...
objarr = np.unique(ic_all.summary['object'])
...
for obj in objarr:
	...
	for filte in _filterarr:
		...
		pattern_head = f"{path_data}/calib_{obs}_{obj}_????????_??????_{filte}_*.head"
		files = sorted(glob.glob(pattern_image))+sorted(glob.glob(pattern_cat)+sorted(glob.glob(pattern_head)))
...
log_text = f"{path_data} {start_localtime} {end_localtime}"
...
channel = '#pipeline'
text = f'[`gpPy`+GPU/{project}-{obsmode}] Processing Complete {obs} {os.path.basename(path_new)} Data ({nobj} objects) with {ncore} cores taking {delt_total:.1f} mins'
