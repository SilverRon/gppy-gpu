...
# %%
newlist = ["/large_data/factory/../obsdata/7DT01/2024-04-22_ToO_gain2750"]
...
for ss, (inim, refim, inmask_image, refmask_image, allmask_image) in enumerate(zip(stacked_images, reference_images, sci_mask_images, ref_mask_images, all_mask_images)):
	if refim != None:
		...
		# hcim = refim.replace("fits", "conv.fits")
		hcim = f"{os.path.dirname(inim)}/{os.path.basename(refim).replace('fits', 'conv.fits')}"
...
for obj in objarr:
	_filterarr = np.unique(ic_all.filter(object=obj).summary['filter'])
	for filte in _filterarr:
		#	Path
		path_destination = f'{path_gal}/{obj}/{obs}/{filte}'
		path_transient_candidate = f"{path_destination}/transient_candidate"
		path_transient_cand_png = f"{path_transient_candidate}/png_image"
		path_transient_cand_fits = f"{path_transient_candidate}/fits_image"

		#	Check save path
		paths = [path_destination, path_transient_candidate, path_transient_cand_png, path_transient_cand_fits]
		for path in paths:
			if not os.path.exists(path):
				os.makedirs(path)

		#	Patterns
		pattern_sci = f"{path_data}/calib_{obs}_{obj}_????????_??????_{filte}_*.sci.fits"
		pattern_ref = f"{path_data}/ref_*_{obj}_*_*_{filte}_*.ref.fits"
		pattern_sub = f"{path_data}/*_{obs}_{obj}_????????_??????_{filte}_*.sub.fits"
		pattern_png = f"{path_data}/*_{obj}_*_*_{filte}_*.*.???.png"
		pattern_cat = f"{path_data}/*_{obj}_*_*_{filte}_*transient.cat"

		#	Files
		files0 = sorted(glob.glob(pattern_sci)+glob.glob(pattern_ref)+glob.glob(pattern_sub))
		files1 = sorted(glob.glob(pattern_png))
		files2 = sorted(glob.glob(pattern_cat))
		# print(f"[{obj} in {filte}-band] {len(files0)} found.")
		# print(f"Move Output Files to {path_destination}")
		#
		with ThreadPoolExecutor() as executor:
			executor.map(lambda file_path: move_file(file_path, path_transient_cand_fits), files0)
		with ThreadPoolExecutor() as executor:
			executor.map(lambda file_path: move_file(file_path, path_transient_cand_png), files1)
		with ThreadPoolExecutor() as executor:
			executor.map(lambda file_path: move_file(file_path, path_transient_candidate), files2)
		#	Other images 
		#	Path
		path_destination = f'{path_gal}/{obj}/{obs}/{filte}'

		#	Check save path
		# paths = [path_destination,]
		# for path in paths:
		# 	if not os.path.exists(path_destination):
		# 		os.makedirs(path_destination)

		#	Patterns
		pattern_image = f"{path_data}/calib_{obs}_{obj}_????????_??????_{filte}_*.fits"
		pattern_cat = f"{path_data}/calib_{obs}_{obj}_????????_??????_{filte}_*.cat"
		pattern_head = f"{path_data}/calib_{obs}_{obj}_????????_??????_{filte}_*.head.bkg"
		pattern_png = f"{path_data}/calib_{obs}_{obj}_????????_??????_{filte}_*.png"
		#	Files
		files = sorted(glob.glob(pattern_image))+sorted(glob.glob(pattern_cat)+sorted(glob.glob(pattern_head))+sorted(glob.glob(pattern_png)))
		print(f"[{obj} in {filte}-band] {len(files)} found.")
		print(f"Move Output Files to {path_destination}")
		#
		with ThreadPoolExecutor() as executor:
			executor.map(lambda file_path: move_file(file_path, path_destination), files)
