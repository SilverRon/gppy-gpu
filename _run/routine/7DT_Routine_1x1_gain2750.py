...
try:
    unit = (sys.argv[1]).upper()  # raises IndexError if fed no arguments
    if not bool(re.match(r"7DT\d{2}", unit)):
    ...
    unit = input(f"7DT## (e.g. 7DT01):").upper()
except Exception as e:
    print(e)
    print(f"Unexpected behavior. Check parameter obs")

print(f"# Observatory : {unit.upper()}")
...
path_base = upaths[
    "path_base"
]  # '/home/snu/gppyTest_dhhyun/factory'  # '/large_data/factory'
path_obsdata = (
    f"{path_base}/../obsdata"
    if upaths["path_obsdata"] == ""
    else upaths["path_obsdata"]
)
path_processed = (
    f"{path_base}/../processed_{n_binning}x{n_binning}_gain2750"
    if upaths["path_processed"] == ""
    else upaths["path_processed"]
)
path_refcat = (
    f"{path_base}/ref_cat" if upaths["path_refcat"] == "" else upaths["path_refcat"]
)
path_ref_scamp = (
    f"{path_base}/ref_scamp"
    if "path_ref_scamp" not in upaths or upaths["path_ref_scamp"] == ""
    else upaths["path_ref_scamp"]
)
path_log = (
    f"{path_base}/log/{unit.lower()}.log"
    if "key" not in upaths or upaths["key"] == ""
    else upaths["path_log"]
)
...
path_factory = f"{path_base}/{unit.lower()}"
...
path_phot_mp = str(
    Path_src / "_phot/gregoryphot_7DT_NxN.py"
)  # './phot/gregoryphot_7DT_NxN.py'
path_phot_sub = str(Path_src / "_phot/gregorydet_7DT_NxN.py")
path_subtraction = str(Path_src / "_util/gregorysubt_7DT.py")
path_find = str(Path_src / "_phot/gregoryfind_7DT.py")
# ------------------------------------------------------------
path_raw = f"{path_obsdata}/{unit.upper()}"
# rawlist = sorted(glob.glob(f'{path_raw}/2???-??-??_gain2750'))
rawlist = [
    os.path.abspath(path)
    for path in sorted(glob.glob(f"{path_raw}/2???-??-??_gain2750"))
]
# ------------------------------------------------------------
...
ccdinfo = tool.getccdinfo(unit, path_obs)
...
for imagetyp in ["zero", "flat", "dark"]:
    path_mframe_imagetyp = f"{path_mframe}/{unit}/{imagetyp}"
...
if not os.path.exists(path_log):
    ...
    # example line
    f.write(
        "/lyman/data1/obsdata/7DT01/1994-10-26_1x1_gain2750,1994-10-26_00:00:00_(KST),1994-10-26_00:01:00_(KST),-\n"
    )
    f.close()
# ------------------------------------------------------------
# 	Constant
...
if len(sys.argv) > 2:
    ...
    newlist = [
        os.path.abspath(s)
        for s in rawlist
        if (s not in datalist) and (s + "/" not in datalist)
    ]
    if len(newlist) == 0:
        print("No new data")
        sys.exit()
...
# 	Log All Prints
# ------------------------------------------------------------
path_save_all_log = f"{os.path.dirname(path_log)}/{unit.upper()}"
...
obsinfo = calib.getobsinfo(unit, obstbl)
...
if "tile" not in upaths or upaths["tile"] == "":
    ...
    try:
        ...
        pattern = re.compile(rf"(?=.*{tile}|FLAT|DARK|BIAS)")
...
if slack_report:
    ...
    channel = "#pipeline"
    text = f"[`gpPy`+GPU/{project}-{obsmode}] Start Processing {unit} {os.path.basename(path_new)} Data ({nobj} objects) with {ncore} cores"
...
# 	Preprocessing
# ------------------------------------------------------------
from _refactor import preprocessing as pp

ic_fdzobj, fdzimlist, objtbl = pp.preproc(
    unit,
    path_data,
    path_mframe,
    path_log,
...
# 	ASTROMETRY
# ------------------------------------------------------------
from _refactor import astrometry as ast
...
# 	Photometry
# ------------------------------------------------------------
from _refactor import photometry as ph
from _refactor import combine as cm

path_new_gphot, timetbl, lines = ph.phot(
    path_data, path_default_gphot, path_phot_mp, ncore, n_binning, timetbl
)

timetbl, stacked_images = cm.stack(
    path_config, path_data, tile_name_pattern, skygrid_table, unit, hdr, timetbl
)

# after stack
timetbl = ph.phot_com(
    path_new_gphot, lines, path_data, path_phot_mp, ncore, n_binning, timetbl
)
...
for stack_image in stacked_images:
    ...
    _reference_images_ps1 = glob.glob(
        f"{path_ref_frame}/ref_PS1_{obj}_*_*_{filte}_0.fits"
    )
    _reference_images_7dt = glob.glob(
        f"{path_ref_frame}/ref_7DT_{obj}_*_*_{filte}_*.fits"
    )
    ...
    if len(_reference_images) > 0:
        ...
        all_mask_image = combine_or_mask(
            sci_mask_image, ref_mask_image, mask_suffix="all_mask.fits"
        )
    else:
        ref_image = None
        sci_mask_image = None
...
# 	Image Subtraction
# ======================================================================
for ss, (inim, refim, inmask_image, refmask_image, allmask_image) in enumerate(
    zip(
        stacked_images,
        reference_images,
        sci_mask_images,
...
for oo, obj in enumerate(objarr):
    ...
    for filte in _filterarr:
        # 	Path to Destination
        path_destination = f"{path_processed}/{obj}/{unit}/{filte}"
        #
        ...
        single_frames = sorted(
            [
                f
                for f in glob.glob(f"{path_data}/calib_*_{obj}_*_*_{filte}_*.fits")
                if not re.search(r"\.com\.", f)
            ]
        )
        ...
        single_phot_catalogs = sorted(
            [
                f
                for f in glob.glob(f"{path_data}/calib_*_{obj}_*_*_{filte}_*.phot.cat")
                if not re.search(r"\.com\.", f)
            ]
        )
        print(f"Phot Catalogs (single): {len(single_phot_catalogs)}")
        single_phot_pngs = sorted(
            [
                f
                for f in glob.glob(f"{path_data}/calib_*_{obj}_*_*_{filte}_*MAG_*.png")
                if not re.search(r"\.com\.", f)
            ]
        )
        ...
        # ------------------------------------------------------------
        stack_frames = sorted(
            glob.glob(f"{path_data}/calib_*_{obj}_*_*_{filte}_*.com.fits")
        )
        print(f"Stack Frames: {len(stack_frames)}")
        stack_phot_catalogs = sorted(
            glob.glob(f"{path_data}/calib_*_{obj}_*_*_{filte}_*.com.phot.cat")
        )
        print(f"Phot Catalogs (stack): {len(stack_phot_catalogs)}")
        stack_phot_pngs = sorted(
            glob.glob(f"{path_data}/calib_*_{obj}_*_*_{filte}_*.com.MAG_*.png")
        )
        ...
        convolved_ref_frames = sorted(
            glob.glob(f"{path_data}/ref_*_{obj}_*_*_{filte}_*.conv.fits")
        )
        print(f"Convolved Ref Frames: {len(convolved_ref_frames)}")
        subt_frames = sorted(
            glob.glob(f"{path_data}/calib_*_{obj}_*_*_{filte}_*.subt.fits")
        )
        print(f"Subt Frames: {len(subt_frames)}")
        subt_phot_catalogs = sorted(
            glob.glob(f"{path_data}/calib_*_{obj}_*_*_{filte}_*.subt.phot.cat")
        )
        ...
        sci_snapshots = sorted(
            glob.glob(f"{path_data}/calib_*_{obj}_*_*_{filte}_*.com.*.sci.fits")
        )
        # 	ref_PS1_T12936_00000000_000000_r_0.conv.437.ref.fits
        ref_snapshots = sorted(
            glob.glob(f"{path_data}/ref_*_{obj}_*_*_{filte}_*.conv.*.ref.fits")
        )
        # 	calib_7DT01_T12936_20240423_034610_r_360.com.subt.437.sub.fits
        sub_snapshots = sorted(
            glob.glob(f"{path_data}/calib_*_{obj}_*_*_{filte}_*.subt.*.sub.fits")
        )
        print(
            f"Snapshots (fits): {len(sci_snapshots)}, {len(ref_snapshots)}, {len(sub_snapshots)}"
        )
        ...
        print(
            f"Snapshots (png): {len(sci_png_snapshots)}, {len(ref_png_snapshots)}, {len(sub_png_snapshots)}"
        )

        # 	calib_7DT01_T09373_20240423_033207_r_360.com.subt.flag.summary.csv
        flag_tables = sorted(
            glob.glob(f"{path_data}//calib_*_{obj}_*_*_{filte}_*.subt.flag.summary.csv")
        )
        print(f"Flag Summary Tables: {flag_tables}")
        # 	calib_7DT01_T12931_20240423_031935_r_360.com.subt.transient.cat
        transient_catalogs = sorted(
            glob.glob(f"{path_data}/calib_*_{obj}_*_*_{filte}_*.com.subt.transient.cat")
        )
        print(f"Transient Catalogs: {len(transient_catalogs)}")
        transient_regions = sorted(
            glob.glob(f"{path_data}/calib_*_{obj}_*_*_{filte}_*.com.subt.transient.reg")
        )
        ...
        files_to_phot = (
            single_phot_catalogs
            + single_phot_pngs
            + stack_phot_catalogs
            + stack_phot_pngs
        )
        files_to_transient = (
            convolved_ref_frames
            + subt_frames
            + subt_phot_catalogs
            + ssf_regions
            + ssf_catalogs
        ...
        files_to_candidate_png = (
            sci_png_snapshots + ref_png_snapshots + sub_png_snapshots
        )
...
if slack_report:
    channel = "#pipeline"
    text = f"[`gpPy`+GPU/{project}-{obsmode}] Processing Complete {unit} {os.path.basename(path_new)} Data ({nobj} objects) with {ncore} cores taking {delt_total/60.:.1f} mins"
