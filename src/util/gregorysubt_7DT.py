...
if not os.path.exists(incat):
	print(f"Photometry catalog does not exist: {incat}")
	incat = f"{os.path.dirname(incat)}/phot/{os.path.basename(incat)}"
...
if os.path.exists(hdim):
	print(f"{hdim} exists. Skip Subtraction Process.")
else:
	print(com)
	os.system(com)
