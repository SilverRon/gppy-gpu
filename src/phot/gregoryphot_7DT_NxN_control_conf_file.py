...
def phot_routine(inim):
	...
	# 'mag_u', 'mag_g', ..., 'mag_z'를 중복 없이 추가
	broad_filters = [f"mag_{broad_filter}" for broad_filter in ['u', 'g', 'r', 'i', 'z']]
	keys_from_refcat.extend(filter(lambda key: key not in keys_from_refcat, broad_filters))
...
# 기본 conf
_conf_default = f"{path_config}/gregoryphot_{n_binning}x{n_binning}.sex"

# 4번째 인자로 override
conf = sys.argv[4] if len(sys.argv) >= 5 and sys.argv[4] else _conf_default
print(f"[INFO] Using CONF_NAME: {conf}")
#------------------------------------------------------------
fail_image_list = []
...
for fail_image in fail_image_list: print(fail_image)
