...
def phot_routine(inim):
	...
	#------------------------------------------------------------
	combine_file_numbers = []
	for key in hdr.keys():
		if 'FILE' in key: 
			combine_file_numbers.append(int(key.replace('FILE', '')))
	N_combine = np.max(combine_file_numbers)
	base_image = hdr[f"FILE{N_combine:0>4}"]
	exptime_combine = float(base_image.split('_')[-1].split('.')[0])
	#
	print(inim, obs, obj, refmagkey, refmagerkey)
	obsdict = tool.getccdinfo(obs, path_obs)
	# gain = obsdict['gain']
	gain_default = 0.779809474945068
	gain = (2/3)*N_combine*gain_default
	# pixscale = obsdict['pixelscale']
	pixscale = 0.505*u.arcsec/u.pixel
	fov = obsdict['fov']
	#------------------------------------------------------------
	#	OUTPUT NAMES
	#------------------------------------------------------------
	...
	if not os.path.exists(ref_gaiaxp_synphot_cat):
		try:
			...
			try:
				print("Crowded Field (m_G < 15.0 mag)")
				reftbl = query.querybox_7dt(racent, decent, refqueryradius, ref_gaiaxp_cat, verbose=True, mode='crowd')
			except:
				print("Too Crowded Field (m_G < 14.0 mag)")
				reftbl = query.querybox_7dt(racent, decent, refqueryradius, ref_gaiaxp_cat, verbose=True, mode='too_crowd')
	...
	presexcom = f"source-extractor -c {conf_simple} {inim} -FILTER_NAME {conv_simple} -STARNNW_NAME {nnw_simple} -PARAMETERS_NAME {param_simple} -CATALOG_NAME {precat} -GAIN {gain} -GAIN_KEY FAKE"
	print(presexcom)
	os.system(presexcom)
