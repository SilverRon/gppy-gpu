...
def phot_routine(inim):
	...
	aperlist = [
		0,
		optaper,
		2*0.6731*peeing.value,
		peeing.value*2,
		peeing.value*3,
		(3*u.arcsecond/pixscale).value,
		(5*u.arcsecond/pixscale).value
		]
	aperdiscription = [
		'MAG_AUTO DIAMETER [pix]',
		'BEST APERTURE DIAMETER in SNR curve [pix]',
		'BEST GAUSSIAN APERTURE DIAMETER [pix]',
		'2*SEEING APERTURE DIAMETER [pix]',
		'3*SEEING APERTURE DIAMETER [pix]',
...
comtbl = vstack([ascii.read(tbl) for tbl in sorted(glob.glob(f'{path_base}/*.phot.summary.dat'))])
...
comtbl.write(f'{path_base}/{tbl_outname}.summary.dat', format='ascii.tab', overwrite=True)
if transient == True:
	targetbl = vstack(trtblist)
	targetbl.write(f'{path_base}/{tbl_outname}.dat', format='ascii.tab', overwrite=True)

print(f'{len(failist)} failed images')
f = open(f'{path_base}/{tbl_outname}.fail.list', 'w')
f.write('image\n')
for failim in failist: f.write(f'{failim}\n')
f.close()
#------------------------------------------------------------
#	Time
#------------------------------------------------------------
